...
`documents`는 SCENE 순서로 다음을 저장한다.
...
Desktop은 manual create와 restore 직전에 dirty SCENE을 flush하고 현재
...
본문 line diff, style diff, 장면별 patch 또는 merge는 제공하지 않는다.
...
Desktop은 dirty SCENE/UI state를 flush한 뒤 one-live Typie editor에 exclusive lock을
...
6. 현재 tree, documents, `workspace.v1` 삭제
7. project/app metadata, documents, tree, `workspace.v1` 재생성
...
해제하지 않는다. commit 뒤 reload가 실패하면 temporary graph를 save하지 못하는 fatal
...
- payload는 압축하지 않는다. Typie snapshot을 base64 JSON으로 저장하므로 원본 BLOB보다
  커진다.
- named snapshot마다 모든 SCENE snapshot을 복제하므로 저장 공간은 snapshot 수와
  project 크기에 비례한다.
...
tree/document restore, auto-before-restore와 failure rollback, CRUD/diff/reopen을 검증한다.
